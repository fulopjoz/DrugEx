#!/bin/bash
#PBS -N ccr2_glide_docking
#PBS -l select=1:ncpus=10:ngpus=0:mem=64gb
#PBS -l walltime=100:00:00
#PBS -m ae
#PBS -q cpu
#
# Schrödinger Glide docking of CCR2 generated ligands via Apptainer.
# Runs as a PBS array job: one chunk per array element.
# Supports both HTVS (default) and SP precision modes.
#
# Prerequisites:
...
#     ccr2_gen/scripts/21_glide_docking_array.sh
#
#   # SP (standard precision, for top HTVS hits):
#   qsub -J 1-${N_CHUNKS} \
#     -v GRID_FILE=/path/to/grid.zip,CHUNK_DIR=/path/to/chunks,RESULTS_DIR=/path/to/results,GLIDE_PRECISION=SP \
#     ccr2_gen/scripts/21_glide_docking_array.sh
#
...
#   GLIDE_PRECISION — HTVS (default) or SP
#   N_WORKERS       — Number of parallel Glide workers (default: 10)

set -euo pipefail

# --- Configuration ---
PRECISION="${GLIDE_PRECISION:-HTVS}"
...
echo "=== CCR2 Glide Docking (${PRECISION}) ==="
...
# Copy glide.py and run_glide.py (precision is now a CLI parameter, no sed patching)
...
echo "=== Done: chunk_${CHUNK_IDX} (${PRECISION}) ==="
