...
#
# PLIP on Glide poses: 53 batches × 1000 molecules. %10 concurrent.
# Run AFTER 41_merge_extract.sh completes (can run parallel with 42).
...
GLIDE_POSES="${DRUGEX_DIR}/ccr2_gen/docking/plip_analysis/full_pipeline/glide_poses"
FINAL_PROFILES="${DRUGEX_DIR}/ccr2_gen/docking/plip_analysis/full_pipeline/glide_profiles"
...
echo "=== PLIP Glide batch ${PBS_ARRAY_INDEX} ($(date)) on $(hostname) ==="

python3 ccr2_gen/docking/plip_analysis/run_plip_batch.py \
    --redock-dir "${GLIDE_POSES}" \
